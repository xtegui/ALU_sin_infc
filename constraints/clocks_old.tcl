...
create_generated_clock -name gen_clk_div2 -divide_by 2 -source [get_pins  {q_div_2*/clocked_on}] -master_clock CLOCK_MAIN [get_pins {q_div_2*/Q}] -add
# Clock dividido por 4 (q_div_4)
create_generated_clock -name gen_clk_div4 -divide_by 2 -source [get_pins  {q_div_4*/clocked_on}] -master_clock gen_clk_div2 [get_pins {q_div_4*/Q}] -add
# Clock dividido por 8 (q_div_8)
create_generated_clock -name gen_clk_div8 -divide_by 2 -source [get_pins  {q_div_8*/clocked_on}] -master_clock gen_clk_div4 [get_pins {q_div_8*/Q}] -add
# Clock seleccionado en la salida del multiplexor de reloj (o_clock_w)
create_generated_clock -name o_clock_w -source [get_ports i_clock] [get_pins u_clock_mux/clk_out]
# Clocks generados para el MUX 
create_generated_clock -name gen_clk_CLOCK_MAIN_MUX -divide_by 1 -source [get_pins {u_clock_mux/I0}] -master_clock CLOCK_MAIN [get_pins {u_clock_mux/Z}] -add
create_generated_clock -name gen_clk_DIV2_CLK_MUX -divide_by 1 -source [get_pins {u_clock_mux/I1}] -master_clock gen_clk_div2 [get_pins {u_clock_mux/Z}] -add
create_generated_clock -name gen_clk_DIV4_CLK_MUX -divide_by 1 -source [get_pins {u_clock_mux/I2}] -master_clock gen_clk_div4 [get_pins {u_clock_mux/Z}] -add
create_generated_clock -name gen_clk_DIV8_CLK_MUX -divide_by 1 -source [get_pins {u_clock_mux/I3}] -master_clock gen_clk_div8 [get_pins {u_clock_mux/Z}] -add

# Configuración de los márgenes (uncertainties)
set_clock_uncertainty -setup 0.2 [get_clocks i_clock]
set_clock_uncertainty -hold 0.1 [get_clocks i_clock]
